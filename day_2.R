...
df_2 
df <- rbind(df_1,df_2) #combine dataframes with rbind oder cbind
df
q <- length(df[,1])
q
...
difftime(("2017-12-15"),Sys.Date(),units="days")
library(raster)
...
plot(prec_ger1) #nice
spplot(prec_ger1) #very nice 
prec_ger2 <- mask(prec_ger1,germany, inverse=F)#inverse 
spplot(prec_ger2)
